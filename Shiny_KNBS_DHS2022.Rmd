...
table28a <- table28a %>%
  ...
  filter(variable!=c("Total", "Number"))

```

```{r}
renderPlot({ggplot(table28a[table28a$Background.characteristic%in%input$agric_own,],mapping=aes(y=value, x=Indicator, fill=Participants)) +
  geom_col(position="dodge") + 
  facet_wrap(~variable) + 
  theme_bw() +
  labs( x = paste0(unique(table28a[table28b$variable!=c("Total", "Number") &table28a$Background.characteristic%in%input$agric_own,]$Background.characteristic), " of respondents"), y="%", fill="Land Ownership")+theme(text=element_text(size=20, face="bold"))}, width=1500, height=400)
