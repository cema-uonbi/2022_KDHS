...
teenpregnancybycountyvisuals <- teenpregnancybycountyvisual %>%
  ...
  theme(text=element_text(size=20, face="bold")) +
  theme(strip.background =element_rect(fill="gray80")) +scale_fill_manual(values=c("darkblue", "darkgreen", "black", "red"))
...
selectInput("nutr1", "Choose background indicators to display:",
              c("Mother's education"="Mother_education",
                "Wealth quintile"="Wealth_quintile"))
```
...
## Child nutrition by background characteristics
...
#else if(input$nutr1){
renderPlot({
  ggplot(nut_total[nut_total$Indicator1%in%input$nutr1,], aes(x=Indicator, y=perc_below2sd, fill=measure))+
...
```{r}
selectInput("nutr2", "Choose nutritional measure to display:",
              c("HFA"="HFA",
                "WFA"="WFA",
                "WFH"="WFH"))
```
