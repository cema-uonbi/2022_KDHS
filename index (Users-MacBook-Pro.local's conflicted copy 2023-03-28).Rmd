...
hiv_discriminatory_att <- read_csv("data/table_13/table13_4_HIV_discriminatory_att_by_background_xtics.csv")

hiv_discriminatory_att_county <- read_csv("data/table_13/table13_4C_HIV_discriminatory_att_by_county.csv")%>%
  mutate(County=recode(County,"Tharaka-Nithi"="Tharaka Nithi", "Elgeyo-Marakwet"="Elgeyo Marakwet","Murang'a"="Muranga"))%>%
  mutate(Proportion=as.numeric(Proportion))%>%
  mutate(Names=paste0(County,", ", Proportion,"%"))

hiv_discriminatory_att_county1 <- full_join(county, hiv_discriminatory_att_county, by=c("Name"="County"))%>%filter(Category%in%"With discriminatory attitudes\n towards people living with HIV")


renderPlot({if(input$hiv_att%in%c("Age", "Education")){
  ggplot(hiv_discriminatory_att[hiv_discriminatory_att$`Background characteristics`%in%input$hiv_att,],aes(x=`Value`, y=Proportion, fill=Category))+
  geom_dotplot(binaxis='y', stackdir='center', binwidth = 4)+
  theme_bw()+
  facet_wrap(~Participants)+
  labs( y="Proportion (%)", title="", x="Age category (in years)", fill="")+
  ...
  else if(input$hiv_att%in%"County"){
    ggplot(hiv_discriminatory_att_county1)+
  geom_sf(data=hiv_discriminatory_att_county1, aes(geometry=geometry), fill=NA)+
  geom_sf(aes(fill=Proportion),color="grey80", size=0.0)+
  theme_void()+
  facet_wrap(~Participants)+
  scale_fill_gradient(low = "white", high = "#8c2d04")+
...
hiv_test <- read_csv("data/table_13/table13_7_HIV_testing_by_background_xtics.csv")

hiv_test$Value <- fct_relevel(hiv_test$Value, "Lowest", "Second", "Middle", "Fourth", "Highest")
hiv_test_county <- read_csv("data/table_13/table13_7C_HIV_testing_by_county.csv")%>%
  mutate(County=recode(County,"Tharaka-Nithi"="Tharaka Nithi", "Elgeyo-Marakwet"="Elgeyo Marakwet","Murang'a"="Muranga"))%>%
  mutate(Proportion=as.numeric(Proportion))%>%
  mutate(Names=paste0(County,", ", Proportion,"%"))

hiv_test_county1 <- full_join(county, hiv_test_county, by=c("Name"="County"))%>%filter(Category%in%"Tested & received results (past 12 months)")



renderPlot({if(input$hiv_test%in%c("Age", "Education")){
  ggplot(hiv_test[hiv_test$`Background characteristics`%in%input$hiv_test,],aes(x=`Value`, y=Proportion, fill=Category))+
  geom_dotplot(binaxis='y', stackdir='center', binwidth = 4)+
  theme_bw()+
  facet_wrap(~Participants)+
  labs( y="Proportion (%)", title="", x="Age category (in years)", fill="")+
  ...
  else if(input$hiv_test%in%"County"){
    ggplot(hiv_test_county1)+
  geom_sf(data=hiv_test_county1, aes(geometry=geometry), fill=NA)+
  geom_sf(aes(fill=Proportion),color="grey80", size=0.0)+
  theme_void()+
  facet_wrap(~Participants)+
  scale_fill_gradient(low = "white", high = "#8c2d04")+
