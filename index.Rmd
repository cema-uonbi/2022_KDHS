...
modernfpusemarriedbywealthquintilevisuals <- read_csv("table8_modernfpusemarriedbywealthquintile.csv") %>%
  mutate(Marital_status="Married") %>%
  mutate(indicator1="Wealtn quintile") %>%
  rename(Indicator="Wealth_quintile") %>%
  mutate(type="Modern")
...
renderPlot({ggplot(table29[table29$Background.characteristic%in%input$physical_violence,]) + 
  ...
  labs( y = 'Percentage(%)',x = 'Marital Status',fill='Experience of Violence', title="Marital Status") +
      theme_bw() + scale_fill_brewer(palette="Paired") + coord_flip()+ theme(text=element_text(size=20, face="bold")) }, width=1400, height=400)
...
renderPlot({
physical_violence <- full_join(county,table29C[table29C$experience%in%input$physical_violence1 ,],by=c("Name"="Background.characteristic")) 
...
renderPlot({
  ...
  labs(fill="Percentage", x="County", y="Percentage")
},width=700, height=400)
