...
renderPlot({
    ggplot(housing[housing$Characteristics%in%input$housing & housing$Participants%in%"Household",], aes(x=Value, y=Proportion, fill=Category))+geom_bar(stat = 'identity', position = position_dodge())+#,aes(tooltip=paste0(Coverage, "%"))) +
  labs(y="Percent (%)", x="")+ylim(0,100)+
...
renderPlot({
    ggplot(cooking[cooking$Characteristics%in%input$cooking & cooking$Participants%in%"Household",], aes(x=Value, y=Proportion, fill=Category))+geom_bar(stat = 'identity', position = position_dodge())+#,aes(tooltip=paste0(Coverage, "%"))) +
  labs(y="Percent (%)", x="")+ylim(0,100)+
    scale_fill_brewer(palette = "Set1")+coord_flip()+
    theme_bw()+theme(text=element_text(size=16, face = "bold")) +
    theme(strip.background =element_rect(fill="darkorange"))+
...
renderGirafe({
  ...
  geom_sf_interactive(data=school_att_county1, aes(geometry=geometry, tooltip=Names), fill=NA)+
  geom_sf(aes(fill=Proportion),color="grey80", size=0.0)+
  theme_void()+
...
respondentsdata <- fread("data/table_2/table2_respondentsdata.csv") %>%
  mutate(Weighted_percent=ifelse(Weighted_percent>0.5, round(Weighted_percent),Weighted_percent))
...
renderGirafe({
  ...
  geom_point(stat = "identity", size= 20)+#, aes(tooltip=paste0(Weighted_percent, "%"))) + 
  theme_bw() +
    #  facet_grid(~Participants) +
  geom_text(aes(label=paste0(Weighted_percent,"%")), hjust = 0.7, size=4, color="black")+scale_color_brewer(palette = "Set1")+ylim(0,100)+
...
#  labs(y="Weighted percent (%)",x=paste0(unique(respondentsdata[respondentsdata$Indicator==input$char,]$Indicator1), " of respondents"), fill="Gender") + 
 ...
 respondents_map2a<-girafe(ggobj = respondents_map2,
  ...
  theme(text=element_text(size=18, face="bold")) + theme(strip.background =element_rect(fill="darkorange"))+
      geom_text(aes(label=label), position=position_stack(vjust = 0.7), size=4, color="white")
  ...
  print(respondents_map3a)
    ...
    background_xtics <- ggplot(background_characteristics_map[background_characteristics_map$group%in%input$char,], aes(fill=weighted_perc))+ geom_sf_interactive(aes(tooltip=names),color="grey80", size=0.0)+ylim(0,100)+
  theme_void()+
...
renderPlot({ggplot(early_childhood[early_childhood$Variable%in%input$early_childhood,],aes(x=Characteristic, y=proportion, fill=category))+
  ...
  theme_bw()+scale_fill_brewer(palette = "Set1")+
    geom_text(aes(label=proportion),position=position_dodge(0.8), hjust=1, vjust=0.2)+
...
renderPlot({if(input$early_childhood_mortality!="County"){ggplot(early_childhood[early_childhood$Variable%in%input$early_childhood,],aes(x=Characteristic, y=proportion, fill=category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+scale_fill_brewer(palette = "Set1")+
    geom_text(aes(label=proportion), position=position_dodge(0.6))+
  labs( y="Per 1000", title="", x="", fill="")+
  theme(text=element_text(size=16, face="bold"))}
  else{
...
antenatal_care$Characteristic <- fct_relevel(antenatal_care$Characteristic, "Lowest", "Second", "Middle", "Fourth", "Highest", "No education", "Primary")
