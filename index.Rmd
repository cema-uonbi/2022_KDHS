...
valueBox(
  value = paste("47%"),
  caption = "of women have tested for HIV in the \nlast 12 months and received the results",
  color = "darkgreen"
)
```
...
valueBox(
  value = paste("39%"),
  caption = "of men have tested for HIV in the \nlast 12 months and received the results",
  color = "darkgreen"
)
```
...
selectInput("hiv_test", "",
              c("Have ever tested for HIV"="ever_tested",
                "Have ever tested & received results"="recieved_results",
                "Tested in the last 12 months and received results"="received_results_last_test"))
...
hiv_testc <- read_csv("table25C .1 and .2_HIV_test_bind.csv")%>%
  ...
  pivot_longer(c(recieved_results,ever_tested,received_results_last_test ),names_to ="indicators", values_to="values") %>%
  mutate(names1=paste0(County,", ", values,"%"))




hiv_testc_men <- full_join(county, hiv_testc[ hiv_testc$Participants%in%"men",], by=c("Name"="County"))
hiv_testc_women <- full_join(county,hiv_testc[ hiv_testc$Participants%in%"Women",], by=c("Name"="County"))
hiv_testc1 <- rbind(hiv_testc_men , hiv_testc_women) %>%
...
The analytics and visualizations app is created with the support of the *Center for Epidemiological Modelling and Analysis* (CEMA) at the University of Nairobi, Kenya.
