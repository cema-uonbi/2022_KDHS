...
renderPlot({
  ...
  geom_point(stat = "identity", size= 20, color = "cyan4") + 
  theme_bw() +
      facet_grid(~Participants) +
  geom_text(aes(label=paste0(Weighted_percent,"%")), hjust = 0.5, size=4, color="white", face = "bold")+
  ...
  geom_col(stat = "identity", size= 10, fill = "cyan4") + coord_flip()+
  theme_bw() +
        facet_grid(~Participants) +
  geom_text(aes(label=Weighted_percent), position=position_stack(vjust = 0.7), size=4, color="white", face = "bold", label.size=0.7)+
  labs(y="Weighted percent (%)",x=paste0(unique(respondentsdata[respondentsdata$Indicator==input$char,]$Indicator1), " of respondents"), fill="Gender") + 
  ...
  theme_bw() +
        facet_grid(~Participants) +
  geom_text(aes(label=Weighted_percent), position=position_stack(vjust = 0.7), size=5, color="white", face = "bold", label.size=0.7)+
  labs(y="Weighted percent (%)",x=paste0(unique(respondentsdata[respondentsdata$Indicator==input$char,]$Indicator1), " of respondents"), fill="Gender") + 
        # labs(y="Weighted percent (%)") + 
        scale_fill_manual(values=c("cyan4", "red4"))+
  theme(text=element_text(size=20, face="bold")) + theme(strip.background =element_rect(fill="darkorange")) } 
    ...
    filter(Indicator==input$char)%>%
    ggplot(aes(x=Grouping, y=Weighted_percent)) + 
  geom_col(stat = "identity", size= 10, fill = "cyan4") + 
  theme_bw() +
        facet_grid(~Participants) +
  geom_text(aes(label=Weighted_percent), position=position_stack(vjust = 0.5), size=8, color="white", face = "bold")+
...
#### **Health Insurance Coverage:** *Health insurance improves access to quality health care by reducing the costs associated with illness, treatment and care incurred by persons and/or families. The graphics show the percentage of de jure household members with insurance and the type of insurance.* 
...
selectInput("disability_domain", "Choose  a disability domain:",
              c("Seeing"="Seeing", 
                "Hearing"="Hearing", 
...
valueBox(
  value = paste("70%"),
  caption = "of the unmarried women men use a contraceptive",
  color = "red"
)
```
...
renderPlot({ggplot(family_planning[family_planning$indicator1%in%input$family_planning & family_planning$type%in%input$fp_type,], aes(x=Indicator, y=Percent, fill=Contraceptive_use)) +
  ...
  theme(text=element_text(size=20, face="bold")) +
  theme(strip.background =element_rect(fill="gray80")) }, width=1400, height=900)
...
table29 <- dplyr::bind_rows(table29_men, table29_women)
table29C <- dplyr::bind_rows(table29C_women,table29C_men) %>%
  select(-Number)%>%
...
```{r}
renderPlot({ggplot(table29[table29$Background.characteristic%in%input$physical_violence,]) + 
  geom_col(mapping=aes(x=Indicator,y=Value,fill=Category),
           position='dodge') + facet_grid(.~Participants) + 
  labs( y = 'Percentage(%)',x = 'Marital Status',fill='Experience of Violence', title="Marital Status") +
...
#physical_violence <- rbind(women, men)
  ...
  ggplot(physical_violence) + geom_sf(aes(fill=value))+
  theme_void()+scale_fill_gradient(low="#ffffcc", high="#bd0026")+facet_grid(.~Participants)+ 
...
renderPlot({
  ...
  geom_bar(stat="identity", aes(fill=Participants)) + coord_flip() + theme_bw() +
...
```{r}
selectInput("vacc1", "",
              c("Fully vaccinated: 12-23 months"="12-23 months",
                "Fully vaccinated: 24-35 months"="24-35 months"))
...
renderPlot({
  ggplot(vaccination_data[vaccination_data$indicator%in%input$vacc_char & !is.na(vaccination_data$indicator1) & vaccination_data$group%in%input$vacc1,],aes(x=Vaccines, y=Percent)) + 
  geom_bar(stat = "identity", position = position_dodge(), fill="cyan4") +
...
[Fertility by residence](#fertility_residence_trend)</br>

[Need for, and demand for family planning](#fp_trend)</br>

[Delivery assistance](#skilled_deliveries_trend)</br>

[Childhood vaccinations](#vaccines_trend)</br>

[Children nutritional status](#nutrition_trend)</br>

[Exclusive breastfeeding](#exclusive_breastfeed_trend)</br>

[Childhood mortality](#mortality_trend)</br>

[Household possession of ITNs](#itn_trend)</br>



fertility_residence_trend {.hidden}
==================
...
# combine data
trends_data <- rbind(trendstotalfertilityrate, trendsinfusevisual, trendsdeliveryassistance, trendschildvaccinations) %>%
  filter(!grouping%in%"Total")

renderPlot({
  trends_data %>% filter(indicator%in%input$trends)%>%
  ggplot(aes(x=Years, y=Percent, color=grouping)) + 
  geom_point(stat = "identity", size= 2.5) +
  scale_x_discrete(limit=c(1989, 1993, 1998, 2003, 2008, 2014, 2022)) + 
  theme_bw() + 
  geom_line(size = 2.0) +
  labs(x="Period (Years)", y=paste0(unique(indicator))) +
  theme(text =element_text(size=18, face="bold"))+
  geom_text(aes(label = Percent), hjust=-0.5, size=4)
},width = 1500, height=800)
...
trendschildnutrition <- read_csv("figure6_trendschildnutrition.csv") %>%
  mutate(lab = str_c(Percent,"%"))

renderPlot({
  trendschildnutrition %>%
  ggplot(aes(x=Years, y=Percent, color=`Nutrition measure`)) + 
  geom_point(stat = "identity", size= 2.5) +
  scale_x_discrete(limit=c(1993, 1998, 2003, 2008, 2014, 2022)) + 
  theme_bw() + 
  geom_line(size = 1.0) +
  ylab("%") + xlab("KDHS years") +
  theme(strip.text.x = element_text(size = 12, colour = "black", face = "bold")) +
  theme(strip.text.y = element_text(size = 12, colour = "black", face = "bold")) +
  theme(axis.title=element_text(size=12, face="bold"), axis.text = element_text(face = "bold", size = rel(1))) +
  theme(legend.text = element_text(size=16, face = "bold"), 
        legend.title = element_text(size=16, face = "bold")) +
  theme(strip.background =element_rect(fill="darkorange"))+
  geom_text(aes(label = lab), hjust=-0.5, size=4)
},width = 1500, height=800)

```

exclusive_breastfeed_trend {.hidden}
==================
...
renderPlot({
  trendsexclusivebreastfed %>%
  ggplot(aes(x=Years, y=Proportion)) + 
  geom_point(stat = "identity", size= 2.5) +
  scale_x_discrete(limit=c(1989, 1993, 1998, 2003, 2008, 2014, 2022)) + 
  theme_bw() + 
  geom_line(size = 1.0) +
  ylab("% of children aged 0-5 months exclusively breastfed") + xlab("KDHS years") +
  theme(strip.text.x = element_text(size = 12, colour = "black", face = "bold")) +
  theme(strip.text.y = element_text(size = 12, colour = "black", face = "bold")) +
  theme(axis.title=element_text(size=12, face="bold"), axis.text = element_text(face = "bold", size = rel(1))) +
  theme(legend.text = element_text(size=16, face = "bold"), 
...
trendschildmortality <- read_csv("figure8_trendschildmortality.csv") %>%
  mutate(lab = str_c(n_per1000))

renderPlot({
  trendschildmortality %>%
  ggplot(aes(x=Years, y=n_per1000, color=Category)) + 
  geom_point(stat = "identity", size= 2.5) +
  scale_x_discrete(limit=c(1989, 1993, 1998, 2003, 2008, 2014, 2022)) + 
  theme_bw() + 
  geom_line(size = 1.0) +
  ylab("Deaths per 1,000 live births") + xlab("KDHS years") +
  theme(strip.text.x = element_text(size = 12, colour = "black", face = "bold")) +
  theme(strip.text.y = element_text(size = 12, colour = "black", face = "bold")) +
  theme(axis.title=element_text(size=12, face="bold"), axis.text = element_text(face = "bold", size = rel(1))) +
  theme(legend.text = element_text(size=16, face = "bold"), 
...
renderPlot({
  trendsitn %>%
  ggplot(aes(x=Years, y=Percent, color=Category)) + 
  geom_point(stat = "identity", size= 2.5) +
  scale_x_discrete(limit=c(1989, 1993, 1998, 2003, 2008, 2014, 2022)) + 
  theme_bw() + 
  geom_line(size = 1.0) +
