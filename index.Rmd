...
radioButtons("quest", "",
             c("Households"="All",
              # "Response Rate"="Response Rate",
               "Participants: Men (15-54)"="Male",
               "Participants: Women (15-49)"="Female"))
...
totalinterviewvisuals <- rbind(interviewvisuals ,participantsinterviews)  %>%
  ...
  mutate(Respondents=recode(Respondents, "Eligible men response rate2"="Response Rate", "Eligible women response rate2"="Response Rate", "Household response rate1"="Response Rate")) %>%
  mutate(Participant=ifelse(Respondents%in%"Response Rate", "Response Rate", Participant)) %>%
...
renderPlot({if(input$quest%in%"Response Rate"){
  ggplot(totalinterviewvisuals[totalinterviewvisuals$Gender%in%input$quest & totalinterviewvisuals$Respondents!="Response Rate",], aes(x=Respondents, y=numbers)) + 
...
[Gender based violence](#gender_based)</br>
...
renderPlot({
  ...
  facet_grid(.~Insurance)+labs(x="", y="Proportion (%)")+
...
renderGirafe({
   disability_map1<-ggplot(data=disabilitydata[disabilitydata$Indicator1==input$disability & disabilitydata$difficulty%in%input$disability_domain,], ) + 
...
renderGirafe({
  nut_map1 <- ggplot(nutc1[nutc1$indicator%in%input$nutr2,], aes(fill=perc_below3sd))+
    ...
    labs(x="", y="",fill="%")+
    theme(text=element_text(size=14)) 
  nut_map1x<-girafe(ggobj = nut_map1,width_svg = 5, height_svg = 5,
                   options = list(
                     opts_hover(css = "fill:#666666;cursor:pointer;"),
                     opts_selection(css = "fill:orange;", type = "multiple"),
                     opts_zoom(max = 4)))
