...
renderGirafe({
  ...
  x <- girafe(ggobj = map3,
                   options = list(
                     opts_hover(css = "fill:#666666;cursor:pointer;"),
                     opts_selection(css = "fill:orange;", type = "multiple"),
                     opts_zoom(max = 4)), width=8, height=6)
    print(x)})
...
renderPlot({
  ggplot(vaccination_data[vaccination_data$indicator%in%input$vacc_char & !is.na(vaccination_data$indicator1),],aes(x=Vaccines, y=Percent, fill=group)) + 
