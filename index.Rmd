...
renderPlot({
  ...
  labs(y="Weighted percent (%)",x=paste0(unique(respondentsdata[respondentsdata$Indicator==input$char,]$Indicator1), " of respondents"), fill="Gender") +scale_color_manual(values=c("cyan4", "red4"))+
       theme(strip.background =element_rect(fill="darkorange")) +
  theme(text=element_text(size=18, face="bold"))}
    else if(input$char%in%"County"){
      respondentsdata%>%
  ...
  theme_bw() +
        facet_grid(~Participants) +
  geom_text(aes(label=Weighted_percent), position=position_stack(vjust = 0.5), size=4, color="white", face = "bold", label.size=0.7)+
  ...
  theme_bw() +
        facet_grid(~Participants) +
  geom_text(aes(label=Weighted_percent), position=position_stack(vjust = 0.5), size=4, fill="white", face = "bold")+
  labs(y="Weighted percent (%)",x=paste0(unique(respondentsdata[respondentsdata$Indicator==input$char,]$Indicator1), " of respondents"), fill="Gender") +scale_fill_manual(values=c("cyan4", "red4"))+
  theme(text=element_text(size=20, face="bold")) +  theme(strip.background =element_rect(fill="darkorange")) 
    }
