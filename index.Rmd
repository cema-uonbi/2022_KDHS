...
disabilitydata$Indicator <- fct_relevel(disabilitydata$Indicator, "Lowest", "Second", "Middle", "Fourth", "Highest",
 ...
 renderGirafe({
  ...
  geom_bar_interactive(aes(x=Indicator, y=difficultyprop, fill=difficulty, tooltip=paste0(difficultyprop, "%")),stat = "identity", position = position_dodge()) +
...
renderGirafe({
  teenage_preg1<-teenpregnancybycountyvisual %>%
  ggplot(aes(x=reorder(County,Percent), y=Percent, fill=Pregnancy)) + 
  geom_bar_interactive(stat = "identity", aes(tooltip=paste0(County,"\n",Percent, "%"))) +
