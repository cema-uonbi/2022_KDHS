...
renderPlot({
  ...
  facet_grid(.~Insurance)+labs(y="Proportion (%)")+
    ...
    theme(strip.background =element_rect(fill="darkorange")) 
  }else{ ggplot(healthinsurance[healthinsurance$Indicator1%in%input$health,], aes(x=Indicator, y=Coverage, fill=Participants))+geom_bar(stat = 'identity', position = position_dodge()) +
  facet_grid(.~Insurance)+labs(y="Proportion (%)")+
